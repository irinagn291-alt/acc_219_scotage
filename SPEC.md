...
**One-line positioning:** Tap the aimed lot so it quits and this month's load falls.

| Field | Value |
| --- | --- |
| Product name | Scotage |
| Bundle identifier | `com.scotage.face` |
| Domain | https://scotage-face.pro |
| Contact URL | https://scotage-face.pro/contact-us |
| Deployment target | iOS 17.0 |
| Swift version | 6.2, strict concurrency `complete` |
| Devices | iPhone and iPad, portrait |
| Interface style | Light |
| Asset prefix | `sct_` |
| User-Agent | `Scotage/1.0 (iOS; +https://scotage-face.pro)` |
...
8. **The app must build clean** with
   `xcodegen generate && xcodebuild -scheme Scotage -destination 'generic/platform=iOS' build`.
...
A ratepayer taps the lot under the hand so that lot files as quit and the monthly load falls.

### 2.1 User flow

1. Tap the lot under the hand on the Ledger face so it files as quit.
2. Watch the needle fall by that lot's period-normalized month amount and cancel-savings rise.
3. Add a lot on the face — amount, period, next charge day — no pushed detail.
4. Open Insights as a sheet to list quits, cooled misses, leftover overrun, and mystery-charge flags.
5. Open Settings as a sheet to set currency, the Scot pin, and a local reminder.

### 2.2 Essential behaviour

- Period-normalized month amount = amount × period.occurrencesPerMonth (weekly = 52/12)
- Lots that bill this month only sit on the face
- Aim-then-quit: only the aimed overrun lot can file as quit
- Cancel-savings = the sum of quit lots' month amounts
- Mystery-charge flag when a lot has no period or shares a next-charge day
- Local only, no bank login

---

## 3. Uniqueness assignment for Scotage

| Axis | Assigned value |
| --- | --- |
| Architecture | **Precept ADT fold (Bare | Aimed | Eased); the face is a fold over Lots; Aim writes a Hand on the live overrun Lot with the largest period-normalized month amount and folds Bare to Aimed; Quit writes a QuitMark when the tapped Lot is the aimed Lot and drops it from the live stack; a miss writes a HoldMark and keeps the Hand; the last true Quit that brings live load to or under the Scot folds Aimed to Eased; Quit on Bare is refused; a second Aim while Aimed is refused; Aim samples a Lot that is not Quitted whose stack sits past the Scot; empty face writes Vacant** |
| UI approach | **UIKit UICollectionView compositional layout diffable data source · material** |
| Naming convention | **Scot-and-lot lexicon (Face, Lot, Scot, Hand, QuitMark, HoldMark)** |
| File organization | **By precept role (Face, Lot, Scot, Hand, QuitMark, HoldMark)** |
| Dependency strategy | **None** |
| Design direction | **spacious · analog-face · duotone** |
| Typography | **SF Pro** |
| Navigation pattern | **Face-locked chrome (the rate face never leaves; Insights and Settings arrive as sheets)** |
| AI art style | **Gouache illustration · abstract** |
| Functional twist | **Aim-then-quit (the hand points at the largest overrun lot; only that tap files a QuitMark)** |
...
**Family** — subscription_load

**Core** — A ratepayer taps the lot under the hand so that lot files as quit and the monthly load falls.

**Audience** — People who already know the month is too heavy and will tap the aimed overrun to drop it, without linking a bank.
...
**Twist** — Aim-then-quit. The Ledger is a rate face, not a beam and not a list. A Scot pin marks the month the user can bear. Lots that bill this month sit as arcs around the rim. The needle is the period-normalized month total; a year inscription multiplies that total by twelve. The hand aims at the largest arc that still sits past the Scot. Tapping that aimed arc files the lot as quit, the needle drops, and cancel-savings rise by that lot's month amount. Tapping any other arc cools it and leaves the hand where it is. After seed the hand already aims at an overrun lot, so the first tap can file. Insights lists quits, cooled misses, leftover overrun, and mystery-charge flags. Settings stores currency, the Scot, and a local reminder. The job on home is quit-the-aimed-lot.

**Why this is not a repeat** — Kentledge seats freed credit on a keep or sink after a drag-cut on a load beam. This product keeps subscription_load math — period-normalized month amount, this-month lots only, cancel-savings, mystery-charge, local, no bank — but the Ledger is a rate face. A Scot pin marks the bearable month. The hand aims at the largest overrun lot; only that tap quits. A miss cools the wrong arc and leaves the hand. Insights lists quits and cooled misses, not a rank of uncut bricks. Not Restante payday wait, not Counterfoil hold tickets, not Palmette pinch, not Castellum pours, and not a three-tab subscription list.

### 3.0a Craft from the shipped portfolio

Full craft is in KNOWLEDGE.md. Follow it. Do not copy type names or layouts.
- Home: Leaf: month-total + upcoming charges + cancel-savings.
- Invariant: monthlyEquivalent = amount × period.occurrencesPerMonth (weekly = 52/12). Forecast charges only in the charge month, not smeared.
- Never: No login to a bank. Period-normalized load is the product.
...
The Face is a Precept fold of Vacant, Bare, Aimed, or Eased over the live Lots. Aim samples one Lot that is not Quitted whose period-normalized month stack sits past the Scot, writes a Hand on the largest such Lot, and folds Bare to Aimed; a second Aim while Aimed is refused. Quit writes a QuitMark only when the tapped Lot is the aimed Lot, drops it from the live stack, and the last true Quit that brings live load to or under the Scot folds Aimed to Eased. A miss writes a HoldMark and keeps the Hand; Quit on Bare is refused; an empty live stack writes Vacant. monthlyEquivalent equals amount multiplied by the period occurrencesPerMonth factor with weekly as 52/12; only Lots whose next charge day falls in the current YYYYMMDD month sit on the Face, and cancel-savings is the sum of QuitMark month amounts.
...
SwiftUI WindowGroup hosts FaceChrome. The Ledger mechanic is one UIKit UICollectionView with compositional layout and NSDiffableDataSource, wrapped once in UIViewControllerRepresentable. NSCollectionLayoutGroup.custom places Lot cells as rim arcs; supplementary views draw the needle, the Scot pin, and the year inscription. Cells use the kit material elevation, not a second shadow language. Insights and Settings are stock SwiftUI sheets built from List, Form, and Button. Adding a Lot expands a rim cell in the same snapshot; there is no pushed UIViewController and no second custom-rendered surface. The ui axis value never appears as a title.

### 3.3 Naming contract

Convention: Scot-and-lot lexicon (Face, Lot, Scot, Hand, QuitMark, HoldMark).

Examples to follow: `Lot`, `Hand`, `QuitMark`, `HoldMark`

### 3.4 Dependency contract

Scotage ships no SPM packages and no vendored sources. project.yml has no packages key. Foundation UIKit and SwiftUI only. The compositional UICollectionView NSDiffableDataSource snapshot Precept fold and FaceDocument encoder stay inside the Scotage target. Do not import AVFoundation Vision or VisionKit. Do not call cgi search pl.

### 3.5 Navigation contract

Face-locked chrome: the rate Face never leaves and there is no TabView. Insights and Settings arrive as sheets from FaceChrome. ReviewScreen today stays on LedgerFace; log presents InsightsSheet; goals presents SettingsSheet. Those three keys open three different screens. A new Lot expands on the rim. Contact URL lives on Settings.

### 3.6 Screen composition contract

Face-locked destinations (Ledger is the rate face; Insights and Settings arrive as sheets; a new Lot expands on the rim; no pushed Detail). Physical screens: LedgerFace (root rate face; ReviewScreen today; needle, Scot pin, rim Lots, add-on-rim composer; Vacant is a full-page empty with EmptyHome cutout, headline, one line, and a full-width Add a lot CTA). InsightsSheet (sheet; ReviewScreen log; QuitMarks, HoldMarks, leftover overrun, mystery-charge flags). SettingsSheet (sheet; ReviewScreen goals; currency, Scot, local reminder, contact URL, reset). Onboarding is the only other full screen. The twist lives on LedgerFace. No Today, Scan, Search, or Goals types. No three-tab bar.
...
Scheme: **By precept role (Face, Lot, Scot, Hand, QuitMark, HoldMark)**

```
Scotage/
  Face/ Lot/ Scot/ Hand/ QuitMark/ HoldMark/ DesignSystem/
  Assets.xcassets/
```
...
- **Subscription** — named per this app's convention.
...
Direction: **spacious · analog-face · duotone**

### 7.1 Palette

| Token | Hex | Use |
| --- | --- | --- |
| `background` | `#EAEFE7` | Screen background |
| `surface` | `#F3F6F3` | Cards, rows, sheets |
| `ink` | `#1D2A14` | Primary text and icons |
| `accent` | `#1A9320` | Primary action, key figure, progress fill |
| `muted` | `#526F3F` | Secondary text, dividers, disabled |
...
Assigned type move is one display size then caption with no third weight on the Ledger. SF Pro via Font.system. Display is the needle month total as tabular figures. Caption is the Scot pin the year inscription and rim Lot labels. At most six steps behind one FaceType accessor. Agency feel is huge type then air then silence. No Font.custom and no fixedSize and never below 12pt. Month amounts cancel-savings and the Scot go through NumberFormatter. Dynamic Type at AX5 may drop the needle one step so it never clips. Day edges use Calendar.current.startOfDay then fold to Int YYYYMMDD.
...
This app's `ui` axis is **UIKit UICollectionView compositional layout diffable data source · material**.
...
If **UIKit UICollectionView compositional layout diffable data source · material** is already fully native (no custom drawing layer), this section is satisfied automatically — there is nothing to confine.
...
Reference system: **spacious** — steal rhythm and restraint, not their colours or logos.

Mood: **Generous whitespace, consistent padding, and grid-based layouts for clean, readable, and breathing interfaces.**.

Home rhythm (`analog-face`, comfortable): A face or ring as the object. Digital chrome stays quiet.

High-end agency: huge type, air, one accent, hairline depth. Layout `analog-face`, density comfortable. Kit 28/14, material, soft card. Palette recipe `duotone`. Press scale 0.97, 140-180ms ease-out. Sheets scale 0.96 to 1 plus fade. Reduce Motion: opacity only. Reduce Motion: fade only. Do not invent a second radius or a second accent.
...
UserDefaults stores one Codable FaceDocument under sct.face.v1. The document holds Lots Scot Hand QuitMarks HoldMarks and the Precept case. FaceStore is the only seam and views never see UserDefaults. A true Quit a Scot edit a new Lot or a reset encodes atomically after the mutation. Aim and a refused Quit do not write. Daykey is Int YYYYMMDD from Calendar.current.startOfDay. Simulator seed behind sct.demo.v1 writes several this-month Lots so live load sits past the Scot and the Hand is already aimed. Never seed on a device.
...
Set `User-Agent: Scotage/1.0 (iOS; +https://scotage-face.pro)` on every request. Never reuse another app's string.
...
## 12. Functional twist: Aim-then-quit (the hand points at the largest overrun lot; only that tap files a QuitMark)

The Ledger is a rate Face: a Scot pin marks the bearable month, live Lots sit as rim arcs, the needle is the period-normalized month total, and a year inscription multiplies that total by twelve. After seed the Hand already aims at the largest overrun Lot so the first tap can file. Tapping that aimed arc writes a QuitMark, drops the Lot from the live stack, and the needle falls while cancel-savings rise by that Lot month amount. Tapping any other arc writes a HoldMark and leaves the Hand. Insights lists QuitMarks, HoldMarks, leftover overrun, and mystery-charge flags. Settings stores currency, the Scot, and a local reminder; the home verb is quit-the-aimed-lot.
...
Art style: **Gouache illustration · abstract**
...
given. Every name carries the `sct_` prefix.
...
| --- | --- | --- | --- | --- |
| 1 | `sct_AppIcon` | 1024x1024 | **NO** | App Store icon. NO alpha channel, NO transparency, NO text, NO rounded corners, NO drop shadow outside the canvas. |
| 2 | `sct_Splash` | 1290x2796 | fill | Launch background. The middle third must stay quiet so the wordmark reads on top. |
| 3 | `sct_Onboarding1` | 1024x1536 | **required cutout** | Onboarding page 1 illustration: what the app is for. |
| 4 | `sct_Onboarding2` | 1024x1536 | **required cutout** | Onboarding page 2 illustration: the main verb. |
| 5 | `sct_Onboarding3` | 1024x1536 | **required cutout** | Onboarding page 3 illustration: why they stay. |
| 6 | `sct_EmptyHome` | 1024x1024 | **required cutout** | Empty state: the home screen has nothing yet. Calm and inviting, never sad. |
| 7 | `sct_EmptyList` | 1024x1024 | **required cutout** | Empty state: a secondary list has no rows. |
| 8 | `sct_CardBackdrop` | 1200x800 | fill | Backdrop art for a primary card. Low contrast so text stays readable. |
| 9 | `sct_ControlFace` | 512x512 | **required cutout** | Custom control artwork used for the primary interactive element. |
| 10 | `sct_TwistHero` | 1024x1024 | **required cutout** | Hero art for the 'Aim-then-quit (the hand points at the largest overrun lot; only that tap files a QuitMark)' feature screen. |
| 11 | `sct_SuccessMark` | 512x512 | **required cutout** | Shown briefly when the primary action succeeds. |
| 12 | `sct_HeaderDecor` | 1200x600 | **required cutout** | Decorative header accent on the main screen. |

### Prompt per asset

**`sct_AppIcon`** — 1024x1024

```
Abstract gouache emblem of a rate face reduced to a disc, a pin, and one aimed arc, filling the canvas edge to edge, no text, no transparency, no rounded-corner mask
```

**`sct_Splash`** — 1290x2796

```
Vertical abstract gouache field of a quiet analog face, calm uncluttered centre band, opaque pigment, paper tooth, no readable text
```

**`sct_Onboarding1`** — 1024x1536

```
Solid gouache lot-token cluster on a short stand, opaque painted clay forms, isolated subject, what the product is, not glass
...
**`sct_Onboarding2`** — 1024x1536

```
Solid gouache hand touching one raised arc on a face disc, mid-gesture quit tap, opaque painted mass, isolated subject, not a wire frame

HARD CUTOUT: isolated SOLID opaque subject on a fully transparent background, occupying the center of the canvas. Real PNG alpha channel. All four corners fully transparent. No square plate, no painted backdrop, no opaque box, no drop shadow that fills the canvas. Not glass, not a hollow frame, not a wire outline, not an empty vitrine — rembg punches through those and the cutout is empty. GenerateImage writes opaque RGB — after copy, convert the PNG to RGBA in place; do not generate it again for alpha.
```

**`sct_Onboarding3`** — 1024x1536

```
Solid gouache face with a lowered needle and stacked quit seals beside it, accumulated meaning, opaque pigment, isolated subject

HARD CUTOUT: isolated SOLID opaque subject on a fully transparent background, occupying the center of the canvas. Real PNG alpha channel. All four corners fully transparent. No square plate, no painted backdrop, no opaque box, no drop shadow that fills the canvas. Not glass, not a hollow frame, not a wire outline, not an empty vitrine — rembg punches through those and the cutout is empty. GenerateImage writes opaque RGB — after copy, convert the PNG to RGBA in place; do not generate it again for alpha.
```

**`sct_EmptyHome`** — 1024x1024

```
Solid closed ceramic lot bowl waiting to be filled, fully opaque clay, isolated subject, not glass, not a hollow ring

HARD CUTOUT: isolated SOLID opaque subject on a fully transparent background, occupying the center of the canvas. Real PNG alpha channel. All four corners fully transparent. No square plate, no painted backdrop, no opaque box, no drop shadow that fills the canvas. Not glass, not a hollow frame, not a wire outline, not an empty vitrine — rembg punches through those and the cutout is empty. GenerateImage writes opaque RGB — after copy, convert the PNG to RGBA in place; do not generate it again for alpha.
```

**`sct_EmptyList`** — 1024x1024

```
Solid folded cloth waiting for ink, opaque fabric block, isolated subject, not a glass plate

HARD CUTOUT: isolated SOLID opaque subject on a fully transparent background, occupying the center of the canvas. Real PNG alpha channel. All four corners fully transparent. No square plate, no painted backdrop, no opaque box, no drop shadow that fills the canvas. Not glass, not a hollow frame, not a wire outline, not an empty vitrine — rembg punches through those and the cutout is empty. GenerateImage writes opaque RGB — after copy, convert the PNG to RGBA in place; do not generate it again for alpha.
```

**`sct_CardBackdrop`** — 1200x800

```
Abstract gouache wash of rim arcs behind a quiet centre, low contrast so ink stays readable, fills the canvas, no cutout
```

**`sct_ControlFace`** — 512x512

```
Solid painted analog-face disc with a raised pin and a short hand, opaque gouache, isolated subject

HARD CUTOUT: isolated SOLID opaque subject on a fully transparent background, occupying the center of the canvas. Real PNG alpha channel. All four corners fully transparent. No square plate, no painted backdrop, no opaque box, no drop shadow that fills the canvas. Not glass, not a hollow frame, not a wire outline, not an empty vitrine — rembg punches through those and the cutout is empty. GenerateImage writes opaque RGB — after copy, convert the PNG to RGBA in place; do not generate it again for alpha.
```

**`sct_TwistHero`** — 1024x1024

```
Solid gouache emblem of a hand aimed at one overrun arc, opaque painted mass, isolated subject

HARD CUTOUT: isolated SOLID opaque subject on a fully transparent background, occupying the center of the canvas. Real PNG alpha channel. All four corners fully transparent. No square plate, no painted backdrop, no opaque box, no drop shadow that fills the canvas. Not glass, not a hollow frame, not a wire outline, not an empty vitrine — rembg punches through those and the cutout is empty. GenerateImage writes opaque RGB — after copy, convert the PNG to RGBA in place; do not generate it again for alpha.
```

**`sct_SuccessMark`** — 512x512

```
Solid stamped quit seal, opaque pigment disc, isolated subject, not a hollow outline

HARD CUTOUT: isolated SOLID opaque subject on a fully transparent background, occupying the center of the canvas. Real PNG alpha channel. All four corners fully transparent. No square plate, no painted backdrop, no opaque box, no drop shadow that fills the canvas. Not glass, not a hollow frame, not a wire outline, not an empty vitrine — rembg punches through those and the cutout is empty. GenerateImage writes opaque RGB — after copy, convert the PNG to RGBA in place; do not generate it again for alpha.
```

**`sct_HeaderDecor`** — 1200x600

```
Wide abstract gouache band of rim arcs and a pin, ornamental painted mass, isolated subject
...
Add a unit test target `ScotageTests` covering at minimum:
...
- [ ] `xcodebuild -scheme Scotage -destination 'generic/platform=iOS' build` succeeds.
...
**Uniqueness**
- [ ] Architecture matches **Precept ADT fold (Bare | Aimed | Eased); the face is a fold over Lots; Aim writes a Hand on the live overrun Lot with the largest period-normalized month amount and folds Bare to Aimed; Quit writes a QuitMark when the tapped Lot is the aimed Lot and drops it from the live stack; a miss writes a HoldMark and keeps the Hand; the last true Quit that brings live load to or under the Scot folds Aimed to Eased; Quit on Bare is refused; a second Aim while Aimed is refused; Aim samples a Lot that is not Quitted whose stack sits past the Scot; empty face writes Vacant** with no leakage across layers.
- [ ] UI approach matches **UIKit UICollectionView compositional layout diffable data source · material**.
- [ ] Custom rendering, if any, is confined to one hero surface (section 7.5).
- [ ] Navigation matches **Face-locked chrome (the rate face never leaves; Insights and Settings arrive as sheets)**.
...
xcodebuild -scheme Scotage -destination 'platform=iOS Simulator,id=<UDID>' test
```
